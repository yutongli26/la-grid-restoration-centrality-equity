...
import numpy as np
import pandas as pd
import seaborn as sns  # noqa: F401 (kept for downstream use / consistent environment)
...
# User Configuration
# ==============================================================================
OUTPUT_ROOT = r"C:\2025-2026 Fall\Network science\Project"

DEVICES_CSV: str = (
    r"C:/2025-2026 Fall/Network science/Project/LA_Substations_WithFragility_UPDATED_CEC.csv"
)
SHAPEFILE_PATH = r"C:/2025 Spring/CE295/Supplemental Data/LA_Tracts_With_Population.shp"
...
CEC_TRANSMISSION_SHP: str = (
    r"C:/2025 Spring/CE295/Transmission_Line_7304850911266214096/TransmissionLine_CEC.shp"
)
CITY_BOUNDARY_SHP = r"C:/2025 Spring/CE295/City_Boundary/City_Boundary.shp"
...
def vis_stage2_topology_with_tracts_latlon():
    ...
    Notes:
      - Keeps original logic; improves structure, naming, and readability only.
      - Uses "smart match" to find a device ID column that overlaps edge IDs.
    """
...
def vis_stage2():
    ...
    # ------------------------------------------------------------------
    # 0) Map-based figures (kept exactly as your pipeline entry points)
    # ------------------------------------------------------------------
    ...
    for scen in SCENARIOS:
        ...
        if os.path.exists(kpi_file):
            ...
            for metric in ["T50", "AUC"]:
                ...
                # Map (plot_map assumed defined elsewhere)
                plot_map(
                    gdf,
                ...
                plt.close(fig)  # keep as-is (even though save_plot already closes)
        ...
        if os.path.exists(rob_file):
            ...
            # [Your modification] Left Y min = 0
            ax1.set_ylim(bottom=0)

            # Apply auto X range
            ax1.set_xlim(0, x_limit)

            # Right axis: avg_degree (or fallback)
            ax2 = ax1.twinx()
            color = "tab:blue"
            ...
            # [Your modification] Right Y min = 0
...
def vis_stage4():
    ...
    for scen in SCENARIOS:
        ...
        for rule in RULES:
            ...
            # LCC fraction curve (requires lcc_fraction in Stage 4 CSV)
            if "lcc_fraction" in df.columns:
                axes[0].plot(
                    df["t"],
                    df["lcc_fraction"],
...
def vis_stage6():
    ...
    # Helpers
    # ------------------------------------------------------------------
    def _find_kpi_csv(folder: str) -> str | None:
        """Pick the most likely Stage-6 KPI summary CSV."""
        cands = []
        for f in os.listdir(folder):
            fl = f.lower()
            if not fl.endswith(".csv"):
                continue
            if ("metric" in fl) or ("kpi" in fl) or ("kpis" in fl):
                cands.append(os.path.join(folder, f))

        if not cands:
            return None

        # Prefer files with more informative keywords; else fall back to largest file
        priority = ["equity", "stage6", "summary", "system", "metrics", "kpis"]
        def score(p):
            name = os.path.basename(p).lower()
            s = sum(1 for kw in priority if kw in name)
            try:
    ...
    def _detect_col(df: pd.DataFrame, names: list[str]) -> str | None:
        """Detect a column by a list of candidate names (case-insensitive)."""
    ...
    def _normalize_rule(s: str) -> str:
        """Normalize rule tokens for matching."""
        s = str(s).strip().lower()
        s = s.replace("-", "_").replace(" ", "_")
        return s

    # ------------------------------------------------------------------
    # 1) Locate KPI/Metrics CSV
    # ------------------------------------------------------------------
    target_file = _find_kpi_csv(stage_dir)
    if not target_file:
        print("  [Skip] No KPI/Metrics CSV found in Stage 6 Output.")
        return
    ...
    scenario_col = _detect_col(df, ["scenario", "scen", "event"])
    rule_col     = _detect_col(df, ["rule", "strategy", "method", "heuristic", "policy"])
    entity_col   = _detect_col(df, ["metric_entity", "entity", "level", "scope"])

    metric_col = _detect_col(df, ["t50", "t_50", "t_50_hr", "t50_hr"])
    if metric_col is None:
        # As a last resort: any column containing "t50"
        t50_like = [c for c in df.columns if "t50" in c.lower()]
        metric_col = t50_like[0] if t50_like else None

    if scenario_col is None or rule_col is None or metric_col is None:
        print(f"  [Error] Stage 6 CSV missing required columns.")
        print(f"         Found columns: {list(df.columns)}")
        print(f"         Detected: scenario={scenario_col}, rule={rule_col}, metric={metric_col}")
        return
    ...
    # ------------------------------------------------------------------
    # 3) Filter to system-level if possible
    # ------------------------------------------------------------------
    df_sys = df.copy()
    if entity_col is not None:
        ent = df_sys[entity_col].astype(str).str.strip().str.lower()
        # Keep common system-like tokens
        system_mask = ent.isin(["system", "grid", "network", "overall", "total", "system_level", "system-level"])
        if system_mask.any():
            df_sys = df_sys[system_mask].copy()

    if df_sys.empty:
        print("  [Warning] Stage 6 KPI CSV has no usable system-level rows after filtering.")
        return

    # If multiple rows per (scenario, rule), aggregate (safer than iloc[0])
    df_sys["_scenario_norm"] = df_sys[scenario_col].astype(str).str.strip()
    df_sys["_rule_norm"] = df_sys[rule_col].astype(str).apply(_normalize_rule)

    agg = (
        df_sys.groupby(["_scenario_norm", "_rule_norm"], as_index=False)[metric_col]
        .mean()
        .rename(columns={metric_col: "Hours"})
    )

    # ------------------------------------------------------------------
    # 4) Assemble plot data: Pop vs SVI
    # ------------------------------------------------------------------
    scen_list = SCENARIOS if "SCENARIOS" in globals() else sorted(agg["_scenario_norm"].unique())

    # Normalize the two rules you care about
    pop_key = _normalize_rule("stage3_mean")
    svi_key = _normalize_rule("stage3_mean_svipop")
    ...
    if not plot_rows:
        ...
        print(f"            Available normalized rules include: {sorted(agg['_rule_norm'].unique())[:20]} ...")
        return
...
# ==============================================================================
# 8️⃣ Stage 8: Typology
# ==============================================================================
def vis_stage8(gdf):
    stage_dir = os.path.join(OUTPUT_ROOT, "Stage 8 Output")
    if not ensure_dir(stage_dir):
        return
    print("--- Visualizing Stage 8 ---")

    csv_path = os.path.join(stage_dir, "clusters_labels_final.csv")
    if not os.path.exists(csv_path):
        print("  [Skip] Stage 8 clusters_labels_final.csv not found.")
        return

    df = pd.read_csv(csv_path)
    if df.empty:
        print("  [Skip] Stage 8 CSV is empty.")
        return
    ...
    if "tract_id" in df.columns:
        ...
        print("  [Skip] 'tract_id' missing in Stage 8 CSV.")
        return

    # Ensure cluster exists
    if "cluster" not in df.columns:
        print("  [Skip] 'cluster' missing in Stage 8 CSV.")
        return
    ...
    if {"PC1", "PC2", "cluster"}.issubset(df.columns):
        ...
        save_plot(fig, stage_dir, "vis_stage8_scatter.png")
    else:
        print("  [Stage 8] PC1/PC2 or cluster missing, skip scatter.")

    # ------------------------------------------------------------------
    # 2) Cluster Map on GIS (DISCRETE, do NOT use plot_map with tab10)
    # ------------------------------------------------------------------
    if gdf is None or gdf.empty:
        print("  [Stage 8] gdf is None/empty, skip map.")
    else:
        try:
            g_map = gdf.copy()
            ...
            if n_clusters == 0:
                print("  [Stage 8] No clusters to map (all NaN).")
            else:
                # categorical codes 0..K-1
                merged["_cluster_cat"] = pd.Categorical(merged["cluster"], categories=clusters, ordered=True)
                merged["_cluster_code"] = merged["_cluster_cat"].cat.codes  # -1 for NaN

                # choose a qualitative cmap
                # tab10 up to 10, else tab20
                cmap_name = "tab10" if n_clusters <= 10 else "tab20"
                cmap = plt.get_cmap(cmap_name, n_clusters)

                fig, ax = plt.subplots(figsize=(10, 10))

                # plot missing first (grey + hatch-like via boundary emphasis)
                missing = merged[merged["_cluster_code"] < 0]
                ...
                observed = merged[merged["_cluster_code"] >= 0]
                observed.plot(
                ...
                save_plot(fig, stage_dir, "vis_stage8_map_clusters.png")
        except Exception as e:
            print(f"  [Stage 8] Failed to draw cluster map: {e}")
    ...
    if not feat_cols:
        print("  [Stage 8] No valid numeric feature columns for heatmap/KDE.")
        return
    ...
    try:
        ...
        save_plot(fig, stage_dir, "vis_stage8_heatmap.png")
    except Exception as e:
        print(f"  [Stage 8] Heatmap failed: {e}")
    ...
    if missing_cols:
        print(f"  [Stage 8] Missing KDE columns (skipped): {missing_cols}")

    if not valid_kde_cols:
        print("  [Stage 8] No valid columns found for KDE profiles.")
        return
    ...
    plt.tight_layout()
    save_plot(fig, stage_dir, "vis_stage8_kde_profiles.png")
