...
LA power grid, following a detailed multi-stage specification. It covers:
...
- Stage 5:   (Optional) GA-Based Scheduling Optimization
- Stage 6:   Consolidation, KPI Calculation & Plotting
- Stage 7:   Transportation Flow & Accessibility Loss (Replica OD)
- Stage 8:   Tract-Level Typology (PCA + K-Means)
"""
...
# --- DEFAULT OUTPUT ROOT (predefined so helpers work during class definition) --
DEFAULT_OUTPUT_ROOT = r"C:\2025-2026 Fall\Network science\Project"

# This module-level variable is used by stage_dir()/stage_file during class definition.
# It can later be overridden at runtime via cfg.OUTPUT_ROOT_OVERRIDE when building dirs.
...
class Config:
    ...
    # --- INPUTS (Paths from user's original script)
    # Use raw strings (r"...") for Windows paths
    DEVICES_CSV: str = (
        r"C:/2025-2026 Fall/Network science/Project/LA_Substations_WithFragility_UPDATED_CEC.csv"
    )
    PGA_CSV: str = r"C:/2025-2026 Fall/Network science/Project/Substations_PGA_IDW_CEC.csv"
    MAP_TRACT_SUB_CSV: str = r"C:/2025 Spring/CE295/Data/tract_to_substation_mapping_CEC.csv"
    CEC_GRAPH_EDGES_CSV: str = r"C:/2025 Spring/CE295/Data/substation_graph_CEC_edges.csv"
    CEC_GRAPH_NODES_CSV: str = r"C:/2025 Spring/CE295/Data/substation_graph_CEC_nodes.csv"
    REPLICA_OD_CSV: str = r"C:\2025-2026 Fall\Network science\replica-ladwp-10_30_25-trips_dataset.csv"
    LA_TRACTS_PATH: str = r"C:/2025 Spring/CE295/Supplemental Data/LA_Tracts_With_Population.shp"
    HOSPITAL_TRACTS_CSV: str = r"C:/2025 Spring/CE295/Data/hospital_with_tract.csv"

    # --- Social Vulnerability Index (tract-level, optional)
    SVI_CSV: str = r"C:/2024-2025 Fall/CE294/Project/LA_Census_Tracts_SOVI_Scores_with_Identifiers.csv"
    ...
    TRANSMISSION_LINES_SHP: str = (
        r"C:/2025 Spring/CE295/Transmission_Line_7304850911266214096/TransmissionLine_CEC.shp"
    )
    MAX_LINE_ENDPOINT_DIST_KM: float = 10.0  # endpoint matching radius

    # --- Travel Time Inputs (Fallbacks will be generated if missing)
    # In the new numbering, precomputed travel times conceptually live with Stage 4
    ...
    # --- OUTPUT ROOT (Mandatory Override)
    OUTPUT_ROOT_OVERRIDE: str = r"C:\2025-2026 Fall\Network science\Project"

    # --- OUTPUT DIRS (Relative to override; follow the new sequential stage numbering)
    #   Stage 1 -> MC fragility sampling
    #   Stage 2 -> Static criticality / percolation
    #   Stage 3 -> Step-recovery simulation (MC mean)
    #   Stage 4 -> Rule-based scheduling baselines
    #   Stage 5 -> (Optional) GA-based scheduling
    #   Stage 6 -> Consolidation, KPIs & plotting
    #   Stage 7 -> Flow / accessibility loss (Replica OD)
    #   Stage 8 -> Typology (PCA + K-Means)
    STAGE2_DIR: str = "Stage 1 Output"
    STAGE25_DIR: str = "Stage 2 Output"
    STAGE3_DIR: str = "Stage 3 Output"
    STAGE4B_DIR: str = "Stage 4 Output"
    STAGE4_DIR: str = "Stage 5 Output"
    STAGE5_DIR: str = "Stage 6 Output"
    STAGE8_DIR: str = "Stage 8 Output"
    REPLICA_DIR: str = "Stage 7 Output"
    SCHEDULE_DIR: str = "schedules"  # Subdir for GA runs
    ...
    N_MC: int = 300
    RNG_SEED: int = 42
    N_CREWS: int = 28  # 7 bases * 3 crews (from user's script)
    N_CORES: int = -1  # Use all available cores for Joblib
    K_NEIGHBORS: int = 5  # Fallback k for KNN graph

    # --- Scheduling Fallback
    # Fallback straight-line velocity (km/h) if travel matrices are missing
    VIRTUAL_VEL_KMH: float = 60.0

    # --- Toggle Stages
    USE_GA: bool = False  # Set to True to run full GA (requires DEAP)
    USE_REPLICA_OD: bool = True
    RUN_STAGE_2: bool = True
    RUN_STAGE_2_5: bool = True
    RUN_STAGE_3: bool = True
    RUN_STAGE_4B: bool = True
    RUN_STAGE_5: bool = True
    RUN_REPLICA: bool = True
    RUN_STAGE_8: bool = True

    # --- Crew Bases (from user's script)
...
def setup_logging(log_file: Path) -> logging.Logger:
    ...
    Notes:
        - Resets any existing handlers on the root logger to avoid duplicate logs.
        - Silences noisy warnings from matplotlib font manager and certain GeoPandas iterator warnings.
    """
...
def make_out_dirs(cfg: Config) -> Dict[str, Path]:
    """
    Create (and ensure) all output directories under cfg.OUTPUT_ROOT_OVERRIDE.

    Returns:
        paths: A dict mapping directory keys to Path objects, including:
            - ROOT
            - STAGE2_DIR, STAGE25_DIR, STAGE3_DIR, STAGE4B_DIR, STAGE4_DIR, STAGE5_DIR, STAGE8_DIR, REPLICA_DIR
            - SCHEDULE_DIR (special subdirectory for GA runs)
    """
    logger = logging.getLogger()
    root = Path(cfg.OUTPUT_ROOT_OVERRIDE)
    ...
    for key in dir_keys:
        dir_path = root / getattr(cfg, key)
        dir_path.mkdir(parents=True, exist_ok=True)
        paths[key] = dir_path
        logger.info(f"Output directory ensured: {dir_path}")

    # Special subdirectory for GA runs (under Stage 5 Output, i.e., cfg.STAGE4_DIR).
    sched_dir = paths["STAGE4_DIR"] / cfg.SCHEDULE_DIR
...
REPAIR_PARAM_LOGNORMAL = {
    # ds: (median_hr, log_std_dev)
    #
    # Note:
    # - HAZUS often provides mean repair times.
    # - For a LogNormal distribution: mean = median * exp(0.5 * sigma^2).
    # - In a deterministic setting (e.g., Stage 3 using representative times),
    #   mapping mean directly to "median" is a simplifying assumption (kept as-is).
...
def load_devices(path: str) -> pd.DataFrame:
    ...
    try:
        ...
        # Key fix: prefer 'HIFLD_ID' to align with the CEC graph files.
...
def load_pga(path: str, scenarios: list) -> pd.DataFrame:
    ...
    # Key fix: include lowercase 'pga_' compatibility.
    for scen in scenarios:
        col_candidates = [
            f"pga_{scen}",      # commonly used in your current files (lowercase)
            f"PGA_{scen}",      # alternative capitalization
            f"PGA_{scen}_IDW",  # common suffix from IDW pipelines
            scen,               # scenario name only
        ]
        ...
        # Standardize into pga_<scenario> (copy, not rename, to preserve original columns if present).
...
def run_stage_0(cfg: Config) -> Dict[str, Any]:
    ...
    # --- 2) Critical fix: aggressively sanitize and (re-)ensure 'id' columns ---
    # Column names can occasionally include trailing spaces (e.g., "id ") or casing issues.
    # We do a defensive cleanup here before merge.
...
# =============================================================================
# Contains: Fragility functions (lognorm), MC sampling logic, run_stage_2 (MC engine)
def sample_damage_states(
    ...
    Assumptions:
        devices_df contains the following columns:
            - mu_DS1..mu_DS4    : underlying normal mean (mu = ln(median PGA))
            - beta_DS1..beta_DS4: underlying normal std dev (sigma)
    """
    logger = logging.getLogger()
...
def damage_to_functionality_and_repair(
    ...
    for ds in range(1, 5):
        mask = ds_samples == ds
        try:
            n_samples_ds = mask.sum()

            if n_samples_ds > 0:
                median, sigma = REPAIR_PARAM_LOGNORMAL.get(ds)
                mu = np.log(median)

                repair_time_samples[mask] = lognorm(s=sigma, scale=np.exp(mu)).rvs(
                    size=n_samples_ds,
                    random_state=rng,
                )
            ...
            # Fallback to the median
            repair_time_samples[mask] = REPAIR_PARAM_LOGNORMAL.get(ds)[0]
...
def _process_mc_chunk(
    ...
    # Use standardized substation_id (do not assume devices_df['id'])
    ...
    return (
        ...
        tract_outage_chunk,
        tract_avg_ds_chunk,
        repair_time_samples,  # (n_devices, n_chunk) for Stage 3
    )
...
def run_stage_2(cfg: Config, stage_0_data: Dict, out_dirs: Dict) -> Dict:
    ...
    # Derive sub_index from the canonical substation_id (do not rely on legacy stage_0_data["sub_index"]).
    sub_index = pd.Index(
    ...
    for scenario in cfg.SCENARIOS:
        ...
        # This file can be very large; we save it per scenario.
        ...
        out_path = out_dirs["STAGE2_DIR"] / f"MC_Device_Damage_Records_{scenario}.csv.gz"
        ...
        df_dev_avg = pd.DataFrame(
            {
                "substation_id": sub_index,  # already standardized (HIFLD_ID / substation_id)
                "scenario": scenario,
                "avg_damage_state": device_avg_ds,
            }
        )

        out_path = out_dirs["STAGE2_DIR"] / f"MC_Device_Damage_AvgDS_{scenario}.csv"
        ...
        out_path = out_dirs["STAGE2_DIR"] / f"MC_TractAvgDS_{scenario}.csv"
        ...
        out_path = out_dirs["STAGE2_DIR"] / f"MC_TractOutage_Prob_{scenario}.csv"
...
# =============================================================================
# Contains: Graph building, Centrality metrics (Impact, Betweenness), Percolation, run_stage_2_5
def build_base_graph(cfg: Config, devices_df: pd.DataFrame, sub_index: pd.Index) -> nx.Graph:
    ...
    # --- 0) Load prebuilt CSV graph if available ------------------------------
    if edges_path and _os.path.exists(edges_path):
        ...
        # Critical fix: aggressively sanitize IDs (string + strip)
        ...
        # 6) Optionally attach lat/lon node attributes from the nodes CSV
...
def run_stage_2_5(cfg: Config, stage_0_data: Dict, out_dirs: Dict) -> Dict:
    """Orchestrate Stage 2.5: graph criticality metrics and percolation curves."""
    if not cfg.RUN_STAGE_2_5:
    ...
    out_dir = out_dirs["STAGE25_DIR"]

    # Use the real CEC transmission network where available; otherwise the graph builder
    # is expected to fall back (e.g., to KNN) or return an empty graph.
...
def kpis_from_series(series_df: pd.DataFrame, t_end: float) -> pd.DataFrame:
    ...
    KPIs:
        - AUC: normalized area under the curve over [t0, t_end]
    ...
    # 2) T50 / T80 (first threshold crossing)
...
def run_stage_3(
    ...
    all_mc_repair_times = stage_2_data.get("all_mc_repair_times")
    ...
    G = stage_2_5_data.get("G", nx.Graph())
...
# =============================================================================
# Contains: Travel matrices, Substation ordering rules, Scheduling engine, run_stage_4B
...
def order_substations(
    ...
    centrality_df = stage_2_5_data.get("centrality_df")
    ...
    def _order_by_metrics(metric_names, ascending_flags=None):
        ...
        if centrality_df is None:
            ...
            return order_substations("random", task_sub_ids, stage_0_data, stage_2_5_data, cfg)
...
def simulate_rule_schedule(
    ...
    # Task queue: only tasks that require repair (order is already assumed to be that set)
...
def run_stage_4B(
    ...
    for scenario in cfg.SCENARIOS:
        ...
        for rule in rules:
            ...
            order = order_substations(rule, task_sub_ids, stage_0_data, stage_2_5_data, cfg)
...
def run_stage_5(
    ...
    stage_3_data: Dict,
    stage_4b_data: Dict,
    stage_4_data: Optional[Dict],
    out_dirs: Dict,
):
    """
    Stage 6: Consolidation & plotting (no GA).
    ...
    logger = logging.getLogger()
    logger.info("=" * 50)
    logger.info("--- STAGE 6: Consolidation & Separate Plotting (No GA, Adaptive X) ---")

    out_dir = out_dirs["STAGE5_DIR"]
    t_grid = np.arange(0, cfg.TIME_END_HR + cfg.DT_HR, cfg.DT_HR)
    ...
    # -------------------------------------------------------------------------
    # 3) Plot styling (GA removed; include all 7 rules)
    # -------------------------------------------------------------------------
    style_config = {
    ...
    # -------------------------------------------------------------------------
    # Helper: single-scenario plot (with adaptive X-axis trimming)
    # -------------------------------------------------------------------------
    def plot_single_scenario(scenario_name: str, data_dict: Dict[str, pd.Series], weight_type: str) -> None:
        ...
        # --- Adaptive X-axis trimming (Smart X-Axis Trimming) ---
        if plotted_series:
            try:
                ...
                if unfinished.any().any():
                    ...
                    limit_t = min(cfg.TIME_END_HR, last_t * 1.25)
        ...
        if weight_type == "Population":
            y_label = "Functionality (%)"
        else:
            y_label = "SVI-Weighted Recovery (%)"
    ...
    for scenario in cfg.SCENARIOS:
        ...
        # ---------------------------------------------------------------------
        # A) Stage 3 theoretical limit (system curve via tract series)
        # ---------------------------------------------------------------------
        ...
        if scenario in s3_curves:
            ...
            kpis = kpis_from_series(
                pd.DataFrame({"system": s3_pop}, index=s3_tract_df.index),
                cfg.TIME_END_HR,
            )
            ...
            if svi_pop_weights is not None:
                ...
                kpis_svi = kpis_from_series(
                    pd.DataFrame({"system": s3_svi}, index=s3_tract_df.index),
                    cfg.TIME_END_HR,
                )
        ...
        # ---------------------------------------------------------------------
        s4b_pop_data = stage_4b_data.get("pop", {}).get(scenario)
        s4b_svi_data = stage_4b_data.get("svi", {}).get(scenario)

        if s4b_pop_data is not None:
            for rule in s4b_pop_data.columns:
                series = s4b_pop_data[rule]
    ...
    if all_kpis:
        all_kpis_df = pd.concat(all_kpis).reset_index()
        all_kpis_df.rename(columns={"index": "metric_entity"}, inplace=True)
...
# ==========================================================================
# [PART 8] Stage 7 & 8: Socio-Economic Impact & Typology Clustering (PCA + K-Means)
# =============================================================================
# Contains: Replica OD Analysis, Feature Engineering, PCA, K-Means Clustering, run_stage_8
def run_replica_od_analysis(cfg, stage_3_data, out_dirs) -> bool:
    """
    Stage 7: Flow / Accessibility Loss (Replica OD).

    Performance-oriented implementation using vectorized operations and grouping,
    while preserving the original logic:
      - Text parsing -> canonical 11-digit tract GEOID (LA County prefix + 6-digit suffix)
      - Flow aggregation (OD pair counts)
      - FLI (Flow Loss Index) and AL (Accessibility Loss) time series per scenario
    """
    import logging
    import os
    from pathlib import Path
    ...
    if not cfg.RUN_REPLICA or not cfg.USE_REPLICA_OD:
        logging.info("--- STAGE 7: Skipped ---")
        return False

    logger = logging.getLogger()
    logger.info("=" * 50)
    logger.info("--- STAGE 7: Flow/Accessibility Loss (Replica OD) [Optimized] ---")

    out_dir = out_dirs["REPLICA_DIR"]
    all_mean_tract_series = stage_3_data.get("all_mean_tract_series")

    if not all_mean_tract_series:
        logger.warning("No Stage 3 data found. Skipping Stage 7.")
        return False

    # -------------------------------------------------------------------------
    # Helper: Vectorized parser (matches parse_replica_text_id logic)
    #   "1200.20 (LA)" -> 1200.20 -> *100 -> 120020 -> "06037120020"
    # -------------------------------------------------------------------------
    def vectorized_parse_replica_id(series: pd.Series) -> pd.Series:
        # 1) Extract leading numeric token (digits + dot) from the start of each string
        s_nums = series.str.extract(r"^([\d\.]+)")[0]

        # 2) Convert to numeric, coerce invalid to NaN, fill with 0
        vals = pd.to_numeric(s_nums, errors="coerce").fillna(0)

        # 3) Convert to 6-digit suffix = round(val * 100) and zfill(6)
        suffixes = (vals * 100).round().astype(int).astype(str).str.zfill(6)

        # 4) Prepend LA County FIPS prefix
        return "06037" + suffixes

    # -------------------------------------------------------------------------
    # Helper: Clean supply tract IDs (remove .0 and pad to 11 digits)
    # -------------------------------------------------------------------------
    def clean_supply_cols(cols) -> list:
        return [str(c).split(".")[0].zfill(11) for c in cols]

    try:
        # =====================================================================
        # 1) Load Replica OD Data (only required columns)
        # =====================================================================
        logger.info("Loading Replica OD data (reading only necessary columns)...")

        use_cols = ["origin_trct_2020", "destination_trct_2020"]
        od_df = pd.read_csv(cfg.REPLICA_OD_CSV, usecols=use_cols, dtype=str)

        logger.info(f"Loaded {len(od_df)} rows. Starting vectorized cleaning...")

        # =====================================================================
        # 2) Parse canonical tract IDs (vectorized)
        # =====================================================================
        od_df["origin_can"] = vectorized_parse_replica_id(od_df["origin_trct_2020"])
        od_df["dest_can"] = vectorized_parse_replica_id(od_df["destination_trct_2020"])

        # =====================================================================
        # 3) Aggregate OD flows (group duplicates)
        # =====================================================================
        logger.info("Aggregating OD flows (grouping duplicates)...")
        flow_df = (
            od_df.groupby(["origin_can", "dest_can"])
            .size()
            .rename("flow")
            .reset_index()
        )
        logger.info(f"Reduced to {len(flow_df)} unique OD pairs.")

        # =====================================================================
        # 4) Determine valid tract IDs from Stage 3 (for fallback / AL domain)
        # =====================================================================
        any_series_df = next(iter(all_mean_tract_series.values()))
        valid_tracts = clean_supply_cols(any_series_df.columns)
        valid_tract_set = set(valid_tracts)

        # =====================================================================
        # 5) Load tract list for AL (preserves your original fallback logic)
        # =====================================================================
        tract_csv_path = r"C:\2025-2026 Fall\Network science\Project\Tracts_Within_Los_Angeles.csv"

        if os.path.exists(tract_csv_path):
            logger.info(f"Loading tract list for AL from CSV: {tract_csv_path}")
            t_df = pd.read_csv(tract_csv_path)

            geoid_col = next(
                (c for c in ["GEOID", "GEOID10", "GEOID20", "tract_id"] if c in t_df.columns),
                None,
            )
            ...
            if geoid_col:
                ...
                al_tracts = list(valid_tract_set)
        else:
            al_tracts = list(valid_tract_set)

        # =====================================================================
        # 6) Compute metrics per scenario
        # =====================================================================
        for scenario, tract_series_df in all_mean_tract_series.items():
            logger.info(f"Replica: Analyzing scenario {scenario}...")

            # Align supply columns to match OD IDs
            supply_df = tract_series_df.copy()
            supply_df.columns = clean_supply_cols(supply_df.columns)

            # Pre-filter supply to only tracts referenced by OD pairs (for FLI)
            needed_tracts = list(set(flow_df["origin_can"]).union(set(flow_df["dest_can"])))
            available_tracts = [t for t in needed_tracts if t in supply_df.columns]

            relevant_supply = supply_df.reindex(columns=available_tracts).fillna(0.0)

            total_trips = flow_df["flow"].sum()
            metrics = []

            # Time-step loop
            for t in relevant_supply.index:
                # 1) Supply vector at time t
                current_supply = relevant_supply.loc[t]

                # 2) Map supply to OD origins/destinations (vectorized lookup)
                s_o = flow_df["origin_can"].map(current_supply).fillna(0.0)
                s_d = flow_df["dest_can"].map(current_supply).fillna(0.0)

                # 3) Flow Loss Index (FLI)
                #    Successful flow = flow * supply(origin) * supply(dest)
                successful_flow = (flow_df["flow"] * s_o * s_d).sum()
                fli = 1.0 - (successful_flow / total_trips) if total_trips > 0 else 0.0

                # 4) Accessibility Loss (AL)
                #    AL is computed over the full LA tract list (not only OD-referenced tracts)
                al_vals = supply_df.loc[t].reindex(al_tracts).fillna(0.0)
                al = 1.0 - al_vals.mean()

                metrics.append({"t": t, "FLI": fli, "AL": al})

            # Save results
            out_path = Path(out_dir) / f"stage7_replica_FLI_AL_{scenario}.csv"
            pd.DataFrame(metrics).to_csv(out_path, index=False)
            logger.info(f"Saved FLI/AL for {scenario}: {out_path}")

        logger.info("--- STAGE 7 Complete ---")
        return True

    except Exception as e:
        logger.exception(f"HM-Replica Processing FAILED: {e}")
        return False
...
def run_stage_8(
    ...
    if not cfg.RUN_STAGE_8:
        logging.info("--- STAGE 8: Skipped ---")
        return {}

    logger = logging.getLogger()
    logger.info("=" * 50)
    logger.info("--- STAGE 8: PCA + K-Means (Final Enhanced) ---")
    out_dir = out_dirs["STAGE8_DIR"]
    ...
    def _normalize_tract_id(series: pd.Series) -> pd.Series:
        """
        Standardize tract_id to your common 10-digit style:
    ...
    prob_frames = []
    for scen in cfg.SCENARIOS:
        prob_file = out_dirs["STAGE2_DIR"] / f"MC_TractOutage_Prob_{scen}.csv"
    ...
    # ---------------------------------------------------------
    cent_path = out_dirs["STAGE25_DIR"] / "impact_centrality_substations.csv"
    mapping_df = stage_0_data.get("mapping_df", None)
    ...
    if cent_path.exists() and mapping_df is not None:
        try:
            ...
            m = mapping_df.copy()
            m["tract_id"] = m["tract_id"].astype(str).str.split(".").str[0]
            m["substation_id"] = m["substation_id"].astype(str).str.strip()
            ...
            if "deg" in m.columns:
                grid_deg = g["deg"].mean()
                df_main["Grid_Centrality"] = df_main["tract_id"].map(grid_deg).fillna(0.0)
            ...
            if "weight" in m.columns:
                ...
                df_main["Redundancy_HHI"] = df_main["tract_id"].map(red_df["Redundancy_HHI"])
                df_main["Redundancy_MaxShare"] = df_main["tract_id"].map(
                    red_df["Redundancy_MaxShare"]
                )

            # 6) Dist_to_CrewYard_km (nearest crew yard)
            lat_cols = [c for c in m.columns if c.lower().startswith("lat")]
            lon_cols = [c for c in m.columns if c.lower().startswith("lon")]

            if lat_cols and lon_cols and getattr(cfg, "CREW_BASES", None):
                lat_col = lat_cols[0]
                lon_col = lon_cols[0]
                tract_xy = g[[lat_col, lon_col]].mean()
                ...
                def _min_dist(row):
                    lat_t, lon_t = row[lat_col], row[lon_col]
                    if pd.isna(lat_t) or pd.isna(lon_t):
                        return np.nan
                    dists = [
                        haversine_km(lat_t, lon_t, ylat, ylon)
                        for (ylat, ylon) in yard_coords
                    ]
                    return float(min(dists)) if dists else np.nan

                tract_xy["Dist_to_CrewYard_km"] = tract_xy.apply(_min_dist, axis=1)
                dist_map = tract_xy["Dist_to_CrewYard_km"].to_dict()
                df_main["Dist_to_CrewYard_km"] = df_main["tract_id"].map(dist_map)

        except Exception as e:
            logger.warning(f"Stage 8: failed to compute grid/logistics features: {e}")
    ...
    if os.path.exists(NRI_FILE_PATH):
        ...
        # Construct tract_id to match your 10-digit style (no leading zero)
    ...
    # Scree plot dedicated to Kaiser criterion (Ev > 1)
    try:
        ...
        plt.tight_layout()
        plt.savefig(out_dir / "pca_scree_plot_eigenvalues.png", dpi=300)
        plt.close()
        logger.info("Saved pca_scree_plot_eigenvalues.png")
    except Exception as e:
        logger.warning(f"Failed to plot PCA Scree Plot: {e}")
    ...
    # Keep only intended output columns; drop everything else
    cols_keep = [
        "tract_id",
        "scenario",
        "cluster",
    ...
    # 4. Summary
    # =========================================================
    target_cols = ["T80", "Init_Prob", "Pre_1970_Ratio", "Pop_Density", "SVI_TOTAL", "EP_POV150", "N_subs"]
    avail_cols = [c for c in target_cols if c in df_main.columns]

    if avail_cols:
        logger.info(
            f"\n=== Final Typology Profiles (k={best_k}) ===\n"
            f"{df_main.groupby('cluster')[avail_cols].mean()}"
        )
    else:
        logger.info("Summary columns missing (SVI/Housing data likely not loaded).")
...
def main() -> None:
    ...
    root_override = Path(cfg.OUTPUT_ROOT_OVERRIDE)
    ...
    try:
        ...
        # Stage 2: Monte Carlo fragility sampling
        stage_2_data = run_stage_2(cfg, stage_0_data, out_dirs)

        # Stage 2.5: Static criticality & topology analysis
        stage_2_5_data = run_stage_2_5(cfg, stage_0_data, out_dirs)
        ...
        # Stage 4B: Rule-based scheduling baselines
        stage_4b_data = run_stage_4B(
            cfg,
            stage_3_data,
            stage_0_data,
            stage_2_5_data,
            stage_2_5_data,  # NOTE: preserved as-is
            out_dirs,
        )

        # Stage 5: Genetic algorithm scheduling (disabled/removed)
        stage_4_data = None
        ...
        # Stage 7: Replica OD flow & accessibility analysis (optional)
        # run_replica_od_analysis(cfg, stage_3_data, out_dirs)

        # Stage 8: Tract typology clustering (PCA + K-Means)
        run_stage_8(
            cfg=cfg,
